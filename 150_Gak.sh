...
## R-script for reading the volume of generated water box/oct
cat <<EOF > saltcount.R
saltcount <- function(){					
args <- commandArgs(trailingOnly = TRUE)		
library("readr")			
file__ <- args[1]			
file_ <- read_file(file__)			
b <- strsplit(file_, "A\\\^3")			
c <- strsplit(b[[1]][1], "Volume:")		
d <- as.numeric(c[[1]][2])			
e <- (d * ((10**-9)**3)) * 0.15 * 6.022*10**23
f <- round(e, digits = 0)		
cat(f)
}						
saltcount()					
...
saveOff ${runname} ${runname}_preprod.lib				        
...
mol=$(Rscript saltcount.R leap.log)
echo 'Sodium atoms added in preproduction:' $mol # for slurm log file

mv leap.log ${runname}_preprod_leap.log

cat <<EOF > ${runname}_preprodsalt.in
source leaprc.protein.ff19SB						 
loadamberparams frcmod.ions1lm_126_iod_opc				 
source leaprc.water.opc						 
loadOff ${runname}_preprod.lib					 
addionsrand ${runname} Na+ $mol						 
addions ${runname} Cl- 0						 
saveamberparm ${runname} ${runname}_preprod.prmtop ${runname}_preprod.inpcrd	 
quit
EOF
tleap -f ${runname}_preprodsalt.in
...
saveOff ${runname} ${runname}.lib						
quit
EOF
tleap -f solvated.in

# Read the volume of water from the tleap.log file, save to variable mol
mol=$(Rscript saltcount.R leap.log)
echo 'Sodium atoms added:' $mol # for slurm log file
